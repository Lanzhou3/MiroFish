...
class OasisAgentProfile:
    ...
    # 消费属性字段
    consumption_level: Optional[str] = None  # 消费能力等级：high/medium/low
    price_sensitivity: Optional[str] = None  # 价格敏感度：high/medium/low
    brand_preference: Optional[str] = None  # 品牌偏好描述
    shopping_habits: Optional[str] = None  # 购物习惯描述
    decision_style: Optional[str] = None  # 决策风格：rational/emotional/mixed
    ...
    def to_reddit_format(self) -> Dict[str, Any]:
        ...
        # 添加消费属性信息（如果有）
        if self.consumption_level:
            profile["consumption_level"] = self.consumption_level
        if self.price_sensitivity:
            profile["price_sensitivity"] = self.price_sensitivity
        if self.brand_preference:
            profile["brand_preference"] = self.brand_preference
        if self.shopping_habits:
            profile["shopping_habits"] = self.shopping_habits
        if self.decision_style:
            profile["decision_style"] = self.decision_style
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "shopping_habits": self.shopping_habits,
...
class OasisProfileGenerator:
    ...
    def generate_profile_from_entity(
        ...
        return OasisAgentProfile(
            ...
            # 消费属性
            consumption_level=profile_data.get("consumption_level"),
            price_sensitivity=profile_data.get("price_sensitivity"),
            brand_preference=profile_data.get("brand_preference"),
            shopping_habits=profile_data.get("shopping_habits"),
...
【消费属性】（用于模拟消费行为和决策）
9. consumption_level: 消费能力等级，必须是以下之一: "high"（高消费能力，追求品质）、"medium"（中等消费能力，平衡品质与价格）、"low"（低消费能力，注重性价比）
10. price_sensitivity: 价格敏感度，必须是以下之一: "high"（对价格非常敏感，会货比三家）、"medium"（适度关注价格）、"low"（价格不是主要考虑因素）
11. brand_preference: 品牌偏好描述（100字内），描述该用户对品牌的偏好倾向，如"偏好国产品牌"、"追求国际大牌"、"无品牌偏好"等
12. shopping_habits: 购物习惯描述（150字内），描述购物频率、渠道偏好、决策周期等
13. decision_style: 决策风格，必须是以下之一: "rational"（理性决策，注重数据对比）、"emotional"（感性决策，受情绪和体验影响）、"mixed"（混合型，视情况而定）

重要:
- 所有字段值必须是字符串或数字，不要使用换行符
- persona必须是一段连贯的文字描述
- 使用中文（除了gender字段必须用英文male/female，以及consumption_level/price_sensitivity/decision_style必须用英文指定值）
- 内容要与实体信息保持一致
- age必须是有效的整数，gender必须是"male"或"female"
- 消费属性要与人设背景相符（如学生通常消费能力较低，企业高管消费能力较高）
"""
...
【消费属性】（用于模拟机构的消费/采购行为）
9. consumption_level: 消费能力等级，必须是以下之一: "high"（高预算）、"medium"（中等预算）、"low"（预算紧张）
10. price_sensitivity: 价格敏感度，必须是以下之一: "high"（严格预算控制）、"medium"（适度预算管理）、"low"（预算充足不敏感）
11. brand_preference: 品牌偏好描述（100字内），描述该机构对供应商品牌的偏好倾向
12. shopping_habits: 采购习惯描述（150字内），描述采购流程、供应商选择标准等
13. decision_style: 决策风格，必须是以下之一: "rational"（理性决策，严格流程）、"emotional"（感性决策，重视关系）、"mixed"（混合型）

重要:
- 所有字段值必须是字符串或数字，不允许null值
- persona必须是一段连贯的文字描述，不要使用换行符
- 使用中文（除了gender字段必须用英文"other"，以及consumption_level/price_sensitivity/decision_style必须用英文指定值）
- age必须是整数30，gender必须是字符串"other"
- 机构账号发言要符合其身份定位
- 消费属性要符合机构性质（如政府机构通常流程规范、企业机构根据规模和行业确定）
"""
    ...
    def _generate_profile_rule_based(
        ...
        if entity_type_lower in ["student", "alumni"]:
            return {
                ...
                # 学生消费属性
                "consumption_level": random.choice(["low", "medium"]),
                "price_sensitivity": random.choice(["medium", "high"]),
                "brand_preference": "注重性价比，对价格敏感，会货比三家后选择性价比高的产品",
                "shopping_habits": "购物频率中等，偏好电商平台，喜欢看评价和比价，决策周期较长",
                "decision_style": random.choice(["rational", "mixed"]),
            }
            ...
            return {
                ...
                # 专家/名人消费属性
                "consumption_level": random.choice(["medium", "high"]),
                "price_sensitivity": random.choice(["low", "medium"]),
                "brand_preference": "偏好知名品牌和高质量产品，对品牌有一定忠诚度",
                "shopping_habits": "购物频率较低但客单价高，注重品质和服务，决策相对快速",
                "decision_style": random.choice(["rational", "mixed"]),
            }
            ...
            return {
                ...
                "brand_preference": "注重专业设备和服务的品牌，倾向于行业领先供应商",
                "shopping_habits": "采购流程规范，需要多方比价和审批，决策周期较长",
            ...
            return {
                ...
                "brand_preference": "严格遵循采购规范，注重合规性，倾向于有资质的供应商",
                "shopping_habits": "采购流程严格，需要公开招标或多轮审批，决策周期很长",
                "decision_style": "rational",
            }
            ...
            return {
                ...
                "consumption_level": "medium",
                "price_sensitivity": "medium",
                "brand_preference": "对品牌无特别偏好，综合考虑价格、质量和口碑",
                "shopping_habits": "购物习惯较为常规，根据具体需求灵活调整",
    ...
    def _save_reddit_json(self, profiles: List[OasisAgentProfile], file_path: str):
        ...
        for idx, profile in enumerate(profiles):
            ...
            if profile.consumption_level:
                item["consumption_level"] = profile.consumption_level
            else:
                item["consumption_level"] = "medium"
            
            if profile.price_sensitivity:
                item["price_sensitivity"] = profile.price_sensitivity
            else:
                item["price_sensitivity"] = "medium"
            
            if profile.brand_preference:
                item["brand_preference"] = profile.brand_preference
            else:
                item["brand_preference"] = "综合考虑价格、质量和口碑，无特殊偏好"
            
            if profile.shopping_habits:
                item["shopping_habits"] = profile.shopping_habits
            else:
                item["shopping_habits"] = "常规购物习惯，根据需求灵活调整"
            
            if profile.decision_style:
                item["decision_style"] = profile.decision_style
            else:
                item["decision_style"] = "mixed"
        ...
        logger.info(f"已保存 {len(profiles)} 个Reddit Profile到 {file_path} (JSON格式，包含user_id和消费属性字段)")
